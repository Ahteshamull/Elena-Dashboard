        ...
        <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 text-primary-900">
              ...
              <p className="text-sm font-bold uppercase tracking-widest text-gray-400">Loading User Details...</p>
            </div>
          ) : isError || !user ? (
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left">
                <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg shrink-0 bg-gray-100 flex items-center justify-center">
                  {user.image ? (
                    <img 
                      src={user.image.startsWith('http') ? user.image : `http://localhost:8005${user.image}`} 
                      alt={user.userName} 
                ...
                <div className="flex-1">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
                    <div>
                      <h3 className="text-3xl font-serif text-primary-900">{user.userName || 'Unknown User'}</h3>
                    ...
                    <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest ${
                      user.status === 'blocked' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'
                    }`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                    ...
                    <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-700">{user.country || 'N/A'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                    ...
                    {user.dateOfBirth && (
                      <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-700">{user.dateOfBirth}</p>
                        </div>
                      </div>
                    )}
                  </div>
